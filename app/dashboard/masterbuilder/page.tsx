...
const saveSelectedCourses = (courses: { termin7: Course[]; termin8: Course[]; termin9: Course[] }) => {
...
export default function AllCoursesPage() {
  ...
  const allCourses = useMemo(() => {
    const courseMap = new Map<string, { course: Omit<Course, "availableTerms">; terms: Set<Term> }>();
  ...
  const paginatedCourses = useMemo(() => {
    return filteredCourses.slice((currentPage - 1) * COURSES_PER_PAGE, currentPage * COURSES_PER_PAGE);
    ...
    <div className="container mx-auto py-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {(["termin7", "termin8", "termin9"] as Term[]).map((termin) => (
          <Card key={termin}>
            <CardHeader>
              <CardTitle className="text-lg">{TERM_LABELS[termin]}</CardTitle>
            </CardHeader>
            <CardContent>
              {selectedCourses[termin].length === 0 ? (
                ...
                <div className="space-y-2">
                  {selectedCourses[termin].map((course) => (
                    <div key={course.course_code} className="flex items-center justify-between p-2 rounded border border-border dark:border-muted">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => removeFromTermin(termin, course.course_code)} className="flex-shrink-0 ml-2">
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                  ))}
        ...
        <div className="col-span-1 md:col-span-3 space-y-6 mb-8">
          <div className="flex flex-col md:flex-row gap-6">
            <ProgressBar current={creditStats.totalCredits} goal={CREDIT_GOAL} label="Valda poäng" />
            <ProgressBar current={creditStats.advancedMediaCredits} goal={ADVANCED_MEDIA_GOAL} label="Avancerad nivå i Medieteknik" />
            <ProgressBar current={creditStats.advancedDataCredits} goal={ADVANCED_DATA_GOAL} label="Avancerad nivå i Datateknik" />
          </div>
        </div>
      </div>

      <PillbuttonContainer selectedFilters={selectedFilters} setSelectedFilters={setSelectedFilters} variant="course-filter" />

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>Alla kurser (Termin 7–9)</span>
            <span className="text-sm font-normal text-muted-foreground">{filteredCourses.length} kurser</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {paginatedCourses.map((course) => {
              ...
              return <CourseCard key={`${course.course_code}-${course.semesterName}`} course={course} isSelected={isSelected} onAdd={addToTermin} />;
            })}
          </div>
