...
export function EditCourse({ course }: { course: CourseJSON }) {
  ...
  }, [course.course_code, studyResults]);
  // TODO:
...
function CourseExaminationMapping({
  ...
  const status: Status = (gradeError || dateError) && (dateFocused || gradeFocused) ? "error" : !gradeError && !dateError && examDate?.length === 8 && examGrade?.length === 1 ? "done" : "none";
  return (
    <>
      <div className="grid grid-cols-[auto_1fr_auto] grid-rows-2 gap-1">
        ...
        <div className="col-start-2 row-start-2 flex gap-32">
          <div className="flex flex-col gap-1 items-start text-sm">
            ...
            {dateError && dateFocused && <p className="text-red-900">{dateError}</p>}
          </div>
          <div className="flex flex-col gap-1 items-start justify-start text-sm">
            ...
            {gradeError && gradeFocused && <p className="text-red-900">{gradeError}</p>}
          </div>
