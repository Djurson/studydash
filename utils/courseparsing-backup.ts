...
async function ExtractCoursesAndExaminations(text: string): Promise<Map<string, Course>> {
  ...
  const pageRegex = /Sida\s+(\d+)\s*\/\s*(\d+)/;

  // Loopar igenom hela text array:en
  for (let i = 0; i < textSplit.length - 2; i++) {
    const line = textSplit[i].trim();

    if (line.includes("Kontrollera intyget på:")) continue;
    if (line.includes("Verifierbart tom:")) continue;
    if (line.includes("Sida 1 / 3")) continue;
    if (line.includes("----------------Page")) continue;
    if (line.includes("ResultatintygUtskriftsdatum")) continue;
    if (line.includes("KodBenämning")) continue;

    const pageMatch = pageRegex.test(line);
